...
console.log('\n2️⃣ Auth Package:');
try {
  const authPath = require.resolve('@chat/auth');
  console.log('   @chat/auth resolved to:', authPath);
  
  // Try to import authOptions
  const { authOptions } = require('@chat/auth');
  console.log('   authOptions imported:', authOptions ? '✅' : '❌');
  console.log('   Providers:', authOptions?.providers?.length || 0);
  
} catch (error) {
  console.log('   Error importing @chat/auth:', error.message);
}
