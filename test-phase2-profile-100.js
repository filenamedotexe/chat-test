...
async function testPhase2Profile100() {
  console.log('👤 PHASE 2: PROFILE PAGE 100% TEST\n');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    ...
    // Test 2: Profile Information Display (using actual headings found)
    ...
    try {
      // Find the change password button in the Security section
      const securitySection = await page.locator('h3:has-text("Security")').locator('..');
      const changePasswordButton = await securitySection.locator('button:has-text("Change Password")').first();
    ...
    try {
      ...
      if (sessionsVisible) {
        // Count session items - they're in a list
        const sessionItems = await page.locator('h2:has-text("Active Sessions") ~ div li').count();
        
        results.sessionManagement.status = sessionItems > 0;
        results.sessionManagement.details = `${sessionItems} active sessions displayed`;
        console.log(`✅ Session management: ${sessionItems} sessions shown`);
      } else {
    ...
    try {
      ...
      if (activityVisible) {
        ...
        const activityItems = await page.locator('h3:has-text("Recent Activities") ~ ul li').count();
        
        results.activityTracking.status = true;
        results.activityTracking.details = `Activity section found with ${activityItems} items`;
        console.log(`✅ Activity tracking: ${activityItems} activities displayed`);
      } else {
    ...
    try {
      ...
      if (permissionsVisible) {
        ...
        const permissionItems = await page.locator('h2:has-text("App Permissions") ~ div li').count();
        
        results.permissionsDisplay.status = true;
        results.permissionsDisplay.details = `Permissions section found with ${permissionItems} apps`;
        console.log(`✅ Permissions displayed with ${permissionItems} apps`);
      } else {
    ...
    await page.screenshot({ path: 'profile-100-error.png' });
  } finally {
    await browser.close();
  }
}
...
// Run the test
testPhase2Profile100().catch(console.error);