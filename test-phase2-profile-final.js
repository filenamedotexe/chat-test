...
async function testPhase2ProfileFinal() {
  console.log('👤 PHASE 2: PROFILE PAGE FINAL TEST\n');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    ...
    try {
      ...
      if (sessionsVisible) {
        // Check if the section is displayed (even if empty)
        const sessionText = await page.textContent('h2:has-text("Active Sessions") ~ div');
        const hasNoSessionsMessage = sessionText.includes('No active sessions') || 
                                    sessionText.includes('currently using JWT');
        
        // For JWT auth, having the section displayed with appropriate message is valid
        results.sessionManagement.status = true;
        results.sessionManagement.details = hasNoSessionsMessage ? 
          'Sessions section displayed (JWT auth - no session records)' : 
          'Sessions section displayed';
        console.log('✅ Session management section properly displayed');
      } else {
    ...
    try {
      ...
      if (activityVisible) {
        // Activity section being displayed is sufficient (may be empty for new users)
    ...
    try {
      ...
      if (permissionsVisible) {
        // Permissions section being displayed is sufficient
    ...
    await page.screenshot({ path: 'profile-final-error.png' });
  } finally {
    await browser.close();
  }
}
...
// Run the test
testPhase2ProfileFinal().catch(console.error);