...
async function testPhase4SettingsCorrected() {
  console.log('⚙️ PHASE 4: SETTINGS PAGE CORRECTED TEST\n');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    ...
    try {
      ...
      const hasChatElements = await page.locator('text=Model').isVisible() ||
                              await page.locator('text=Temperature').isVisible();
      
      if (hasModelSelect || hasTemperature || hasChatElements) {
        // Save settings
        const saveButton = await page.locator('button:has-text("Save")').first();
        if (await saveButton.isVisible()) {
          await saveButton.click();
    ...
    await page.screenshot({ path: 'settings-corrected-error.png' });
  } finally {
    await browser.close();
  }
}
...
// Run the test
testPhase4SettingsCorrected().catch(console.error);